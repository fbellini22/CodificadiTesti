...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <xsl:template match="tei:supplied" mode="#default">
        <span class='supplied'>
            <xsl:apply-templates select="tei:supplied"/><xsl:value-of select="."/>
        </span>
    </xsl:template>
    ...
    <!-- Primo template per gestire i nomi abbreviati -->
    <xsl:template match="tei:persName[tei:choice/tei:abbr]">
        <span class='name abbreviation'>
            <xsl:apply-templates select="tei:persName"/>
            <xsl:value-of select="."/>
        </span>
    </xsl:template>

    <!-- Secondo template per gestire i nomi non abbreviati -->
    <xsl:template match="tei:persName[not(tei:choice/tei:abbr)]">
        <span class='name'>
            <xsl:apply-templates select="tei:persName"/>
            <xsl:value-of select="."/>
        </span>
